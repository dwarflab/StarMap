    ...
    int* code)
{
    ...
    *code = 0;
    ...
    if (tile && tile->loader) {
        ...
        cache_set_cost(g_cache, &key, sizeof(key), sizeof(tile_t) + tile->loader->cost);
        free(tile->loader);
        tile->loader = NULL;
        printf("current cache size is %d \n", cache_get_current_size(g_cache));
    }
    if (tile) {
        *code = 200;
        return tile;
    ...
    // won't have to search for it again.
        ...
        if (!(flags & HIPS_LOAD_IN_THREAD)) {
            ...
            cache_set_cost(g_cache, &key, sizeof(key), sizeof(*tile) + cost);
        }
        else {
