...
{
    const double label_color[4] = {0.87, 0.87, 1, 1};
...
static int planets_render(obj_t *obj, const painter_t *painter)
{
    ...
    if (!planets->earth_shadow_tex) {
        planets->earth_shadow_tex = texture_from_url("asset://textures/earth_shadow.png", TF_LAZY_LOAD);
    }

    if (!planets->halo_tex) {
        planets->halo_tex = texture_from_url("asset://textures/halo.png", TF_LAZY_LOAD);
    }
    ...
    PLANETS_ITER(planets, p) {
        if (strcmp(p->name, "saturn") == 0) {
            p->rings.tex = texture_from_url("textures/saturn_rings.png", TF_LAZY_LOAD);
        }
...
static int planets_clear_data_source(obj_t* obj) {
    printf("planets_clear_data_source start\n");
    planets_t* planets = (planets_t*)obj;
    planet_t* p;
    char data_url[256];
    PLANETS_ITER(planets, p) {
        if(p->hips && p->hips->url){
            strcpy(data_url, p->hips->url);
            hips_delete(p->hips);
            p->hips = hips_create(data_url, 0, NULL);
            hips_set_frame(p->hips, FRAME_ICRF);
        }

        if (p->rings.tex) {
            texture_release(p->rings.tex);
            p->rings.tex = 0;
        }
    }

    if (planets->default_hips && planets->default_hips->url) {
        strcpy(data_url, planets->default_hips->url);
        hips_delete(planets->default_hips);
        planets->default_hips = hips_create(data_url, 0, NULL);
        hips_set_frame(planets->default_hips, FRAME_ICRF);
    }

    if (planets->earth_shadow_tex) {
        texture_release(planets->earth_shadow_tex);
        planets->earth_shadow_tex = 0;
    }

    if (planets->halo_tex) {
        texture_release(planets->halo_tex);
        planets->halo_tex = 0;
    }
}
...
static json_value *planet_get_json_data(const obj_t *obj)
{
    ...
    if (planet->parent) {
        ...
        json_value* parents_jv = json_array_new(0);
        while (cur_planet->parent)
        {
            json_array_push(parents_jv, json_string_new(cur_planet->parent->name));
            cur_planet = cur_planet->parent;
        }
        json_object_push(ret, "parents", parents_jv);
        json_object_push(ret, "parent", json_string_new(planet->parent->name));
    }
    else {
        json_object_push(ret, "parents", json_null_new());
        json_object_push(ret, "parent", json_null_new());
    }
...
static obj_klass_t planets_klass = {
    ...
    .clear_data_source = planets_clear_data_source,
    .render_order = 30,
