...
static json_value *dso_get_json_data(const obj_t *obj)
{
    ...
    char buf[256];
    const char* res = skycultures_get_label(names, buf, sizeof(buf));
    if (res) {
        json_object_push(ret, "native", json_string_new(res));
    }
    else {
                               ...
                               void *user)
{
    ...
    *out = NULL;
    ...
    if (json) {
        ...
        if (children_mask != -1) {
            printf("children_mask : %d", children_mask);
            *transparency = (~children_mask) & 15;
        }

        printf("dso children_mask is \n%d\n", children_mask);
    }
...
static int on_db_tile_loaded(const json_value* json, void* user) {
    tile_t* tile;
    dso_t* s;
    int i, j, version, data_ofs = 0, flags, row_size, order, pix;
    int children_mask;
    char morpho[32], ids[256] = {};
    double bmag, temp_mag, tmp_ra, tmp_de, tmp_smax, tmp_smin, tmp_angle;
    void* tile_data;
        ...
        for (int k = 0; k < val_length; k++)
        {
            ...
            if (strcmp(name, "type") == 0) {
                ...
                if (val->u.object.values[k].value->type == json_double) {
                    temp_mag = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "bmag") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    bmag = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "ra") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    tmp_ra = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "de") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    tmp_de = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "smax") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    tmp_smax = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "smin") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    tmp_smin = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "angle") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    tmp_angle = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "morpho") == 0) {
                if (val->u.object.values[k].value->type == json_string) {
                    strcpy(morpho, val->u.object.values[k].value->u.string.ptr);
                }
            }
...
static void *dsos_create_tile(void *user, int order, int pix,
                              const void *data,
                               ...
                               bool longer_label)
{
    ...
    while (*names) {
        ...
        if (len < best_name_len) {
            best_name_len = len;
            snprintf(best_name, sizeof(best_name), "%s", out);
        }
...
static int dsos_clear_data_source(obj_t* obj) {
    dsos_t* dsos = (dsos_t*)obj;
    survey_t* survy, * survy_tmp;
    DL_FOREACH_SAFE(dsos->surveys, survy, survy_tmp) {
        DL_DELETE(dsos->surveys, survy);
        free(survy);
    }
    return 0;
}
...
static obj_klass_t dsos_klass = {
    ...
    .clear_data_source = dsos_clear_data_source,
    .render_order = 25,
