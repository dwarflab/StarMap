...
static int star_init(obj_t *obj, json_value *args)
{
    ...
    type = json_get_attr(args, "type", json_string);
    if (type) {
        strcpy(star->obj.type, type->u.string.ptr);
    }
...
static json_value *star_get_json_data(const obj_t *obj)
{
    ...
    if (star->hip == 0) {
        json_object_push(ret, "native", json_null_new());
    }
    else {
        char buf[256];
        snprintf(hip_buf, sizeof(hip_buf), "HIP %d", star->hip);
        name = skycultures_get_label(hip_buf, buf, sizeof(buf));
        if (name) {
            printf("name:%s\n", name);
            json_object_push(ret, "native", json_string_new(name));
        }
        else {
            json_object_push(ret, "native", json_null_new());
        }
...
static int on_db_tile_loaded(const json_value* json,void* user) {
    survey_t* survey = USER_GET(user, 0);
    tile_t** out = USER_GET(user, 1); // Receive the tile.
    int* transparency = USER_GET(user, 2);
    double vmag, gmag, ra, de, pra, pde, plx, bv, epoch;
    ...
    *out = NULL;
    ...
    json_value* data_json = json->u.object.values[0].value;
    json_value* header_json = json->u.object.values[1].value;

    int nb = data_json->u.array.length;
    ...
    for (int i = 0; i < nb; i++)
    {
        ...
        json_value* val = data_json->u.array.values[i];
        int val_length = val->u.object.length;

        for (int k = 0; k < val_length; k++)
        {
            const char* name = val->u.object.values[k].name;

            if (strcmp(name, "type") == 0) {
                if (val->u.object.values[k].value->type == json_string) {
                    strcpy(s->obj.type, val->u.object.values[k].value->u.string.ptr);
                }
            }
            else if (strcmp(name, "gaia") == 0) {
                if (val->u.object.values[k].value->type == json_integer) {
                    s->gaia = val->u.object.values[k].value->u.integer;
                }
            }
            else if (strcmp(name, "hip") == 0) {
                if (val->u.object.values[k].value->type == json_integer) {
                    s->hip = val->u.object.values[k].value->u.integer;
                }
            }
            else if (strcmp(name, "vmag") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    vmag = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "gmag") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    gmag = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "ra") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    ra = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "de") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    de = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "plx") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    plx = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "pra") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    pra = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "pde") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    pde = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "epoc") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    epoch = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "bv") == 0) {
                if (val->u.object.values[k].value->type == json_double) {
                    bv = val->u.object.values[k].value->u.dbl;
                }
            }
            else if (strcmp(name, "ids") == 0) {
                if (val->u.object.values[k].value->type == json_string) {
                    strcpy(ids, val->u.object.values[k].value->u.string.ptr);
                }
            }
            else if (strcmp(name, "spec") == 0) {
                if (val->u.object.values[k].value->type == json_string) {
                    strcpy(sp_type, val->u.object.values[k].value->u.string.ptr);
                }
            }
        }
        assert(!isnan(ra));
    ...
    if (header_json->type == json_object) {
        children_mask = json_get_attr_i(header_json, "children_mask", -1);
        if (children_mask != -1) {
            *transparency = (~children_mask) & 15;
        }
    }
        ...
        int *cost, int *transparency)
{
    ...
    eph_load(data, size, USER_PASS(survey, &tile, transparency),
             on_file_tile_loaded);
    if (tile) {
        *cost = tile->nb * sizeof(*tile->sources);
        printf("star load,order %d,pix %d,nb is %d \n", order, pix, tile->nb);
    } 
    return tile;
}
                        ...
                        bool sync, int *code)
{
    ...
    //printf("get_tile flags is %d,order is %d,pix is %d,code is %d,tile is %d\n", flags, order, pix, code,tile);
...
static int stars_clear_data_source(obj_t* obj) {
    stars_t* stars = (stars_t*)obj;
    survey_t* survy, * survy_tmp;
    DL_FOREACH_SAFE(stars->surveys, survy, survy_tmp) {
        DL_DELETE(stars->surveys, survy);
        free(survy);
    }
    return 0;
}
...
static obj_klass_t stars_klass = {
    ...
    .clear_data_source = stars_clear_data_source,
    .render_order   = 20,
