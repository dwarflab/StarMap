...
static int skycultures_clear_data_source(obj_t* obj) {
    skycultures_t* cults = (void*)obj;
    obj_t *child,*tmp;

    DL_FOREACH_SAFE(cults->obj.children, child, tmp) {
        module_remove(&cults->obj, child);
    }
}
...
static obj_klass_t skycultures_klass = {
    ...
    .clear_data_source = skycultures_clear_data_source,
