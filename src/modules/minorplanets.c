...
static int mplanets_clear_data_source(obj_t* obj) {
    mplanets_t* mplanets = (void*)obj;
    if (!mplanets->parsed) {
        return 0;
    }
    obj_t* cst;
    obj_t* tmp;
    DL_FOREACH_SAFE(mplanets->obj.children, cst, tmp) {
        module_remove(&mplanets->obj, cst);
    }
    mplanets->parsed = false;

    free(mplanets->source_url);
    mplanets->source_url = NULL;
    return 0;
}
...
static obj_klass_t mplanets_klass = {
    ...
    .clear_data_source = mplanets_clear_data_source,
