...
{
    printf("Click Point:%f,%f\n", gest->pos[0], gest->pos[1]);
    obj_t *obj;
    bool r = false;
    if (core->on_click)
...
static obj_klass_t movements_klass = {
    ...
    .attributes = (attribute_t[]){
        PROPERTY(enable_pan, TYPE_BOOL, MEMBER(movements_t, gest_pan.enable)),
        PROPERTY(enable_click, TYPE_BOOL, MEMBER(movements_t, gest_click.enable)),
        PROPERTY(enable_pinch, TYPE_BOOL, MEMBER(movements_t, gest_pinch.enable)),
    }
};
