...
static int constellation_init(obj_t *obj, json_value *args)
{
    ...
    printf("constellation_set_image execute\n");
...
static int constellations_clear_data_source(obj_t* obj) {
    printf("constellations_clear_data_source start\n");
    constellation_t* con;
...
static int constellations_reload_data_source(obj_t* obj) {
    constellation_t* con;
    MODULE_ITER(obj, con, "constellation") {
        constellation_set_image(&con->obj);
    }
}
...
static json_value *constellation_get_json_data(const obj_t *obj)
{
    ...
    char buf[256];
    const char* res = skycultures_get_label(con->info.id, buf, sizeof(buf));
    if (res) {
...
static obj_klass_t constellations_klass = {
    ...
    .reload_data_source = constellations_reload_data_source,
    .clear_data_source = constellations_clear_data_source,
    .render_order = 25,
    .attributes = (attribute_t[]) {
