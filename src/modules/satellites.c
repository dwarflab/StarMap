...
static int satellites_clear_data_source(obj_t* obj) {
    satellites_t* sats = (void*)obj;
    if (!sats->loaded) {
        return 0;
    }

    //satellite_t *child,*tmp2;
    //DL_FOREACH_SAFE2(sats->visibles, child, tmp2, visible_next) {
    ...
    obj_t* cst;
    obj_t* tmp;
    DL_FOREACH_SAFE(sats->obj.children, cst, tmp) {
        module_remove(&sats->obj, cst);
    }
...
static obj_klass_t satellites_klass = {
    ...
    .clear_data_source = satellites_clear_data_source,
