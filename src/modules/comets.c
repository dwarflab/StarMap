...
static int comet_init(obj_t *obj, json_value *args)
{
    ...
    if (args) {
        int peri_y, peri_m, epoch_y, epoch_m, epoch_d;
        ...
        if (r1) {
            ...
            //"id,number,orbit_type,absolute_mag,slope_param,perihelion_time,inclination,ascending_node_lon,perihelion_arg,perihelion_dist,eccentricity,desig"

            double absolute_mag, slope_param, perihelion_time, inclination, ascending_node_lon, perihelion_arg, perihelion_dist, eccentricity;

            r2 = jcon_parse(args, "{",
                "absolute_mag", JCON_DOUBLE(absolute_mag, 0),
                "slope_param", JCON_DOUBLE(slope_param, 0),
                "perihelion_time", JCON_DOUBLE(perihelion_time, 0),
                "inclination", JCON_DOUBLE(inclination, 0),
                "ascending_node_lon", JCON_DOUBLE(ascending_node_lon, 0),
                "perihelion_arg", JCON_DOUBLE(perihelion_arg, 0),
                "perihelion_dist", JCON_DOUBLE(perihelion_dist, 0),
                "eccentricity", JCON_DOUBLE(eccentricity, 0),
                "}");

            if (r2) {
                LOG_E("Cannot parse simple comet json data.");
                assert(false);
               return -1;
            }

            comet->h = absolute_mag;
            comet->g = slope_param;
            comet->orbit.d = perihelion_time;
            comet->orbit.i = inclination;
            comet->orbit.o = ascending_node_lon;
            comet->orbit.w = perihelion_arg;
            comet->orbit.q = perihelion_dist;
            comet->orbit.e = eccentricity;
            comet->pvo[0][0] = NAN;
        }
        else {
...
static int comets_clear_data_source(obj_t* obj) {
    comets_t* comets = (void*)obj;
    if (!comets->parsed) {
        return 0;
    }
    ...
    obj_t* cst;
    obj_t* tmp;
    DL_FOREACH_SAFE(comets->obj.children, cst, tmp) {
        module_remove(&comets->obj, cst);
    }
    comets->parsed = false;

    free(comets->source_url);
    comets->source_url = NULL;

    comets->render_current = NULL;
    comets->visibles = NULL;
    return 0;
}
...
static int comets_update(obj_t *obj, double dt)
{
    ...
    is_file = strstr(comets->source_url, "http") != comets->source_url;

    if (is_file) {
        FILE* file = fopen(comets->source_url, "r");
        if (file) {
    ...
    LOG_I("source url:%s ,data size:%d,is_file:%d\n", comets->source_url, size,is_file);
...
static obj_klass_t comets_klass = {
    ...
    .clear_data_source = comets_clear_data_source,
