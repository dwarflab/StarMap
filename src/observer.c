...
static json_value* observer_get_radec(obj_t* obj, const attribute_t* attr,
                                const json_value* args)
{
    observer_t* obs = (observer_t*)obj;
    double v[4], b[4];
    ...
    vec4_set(b, 0, 0, 0, 0);
    vec3_from_sphe(obs->yaw, obs->pitch, v);
    convert_framev4(obs, FRAME_OBSERVED, FRAME_ICRF, v, b);
    return args_value_new(TYPE_V3, b);
}
...
static obj_klass_t observer_klass = {
    ...
    .attributes = (attribute_t[]) {
        ...
        PROPERTY(radec,TYPE_V3, .fn = observer_get_radec),
