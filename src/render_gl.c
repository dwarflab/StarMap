...
static int del_grid_cache(void* data) {
    free(data);
}
    ...
    *should_delete = !can_cache;
    ...
    if (can_cache) {
        cache_add(rend->grid_cache, &key, sizeof(key),
                  grid, sizeof(*grid) * n * n, del_grid_cache);
    }
        ...
        const double pos[2], double bounds[4])
{
    ...
    //    printf("fbounds:%f,%f,%f,%f\n", fbounds[0], fbounds[1], fbounds[2], fbounds[3]);
    //    printf("w:%f,h:%f\n", w, h);
    //}
...
static void item_text_render(renderer_t *rend, const item_t *item)
{
    ...
    //        item->text.text, NULL);
    nvgText(rend->vg, bounds[0] + w / 2, roundf(bounds[1] - descender), item->text.text, NULL);

    //printf("text:%s,bounds:%f,%f,%f,%f\n", item->text.text,bounds[0], bounds[1], bounds[2], bounds[3]);
...
void core_add_defualt_font(renderer_t* rend, const char* name, const char* url)
{
    rend = rend ? : (void*)core->rend;
    ...
    int size = 0,id;
    const char* data = asset_get_data2(url, ASSET_USED_ONCE, &size, NULL);
    assert(data);
    ...
    {
        rend->fonts[i].id = id;
        rend->fonts[i].is_default_font = false;
    }
}

void core_set_fallback_font(renderer_t* rend, const char* name, const char* url)
{
    if (!rend->fonts[0].id) {
        return;
    }
    ...
    nvgAddFallbackFontId(rend->vg, rend->fonts[0].id, id);
}
...
void render_clear(renderer_t* rend) {
    if (!rend) {
        return;
    }
    if (rend->white_tex) {
        texture_release(rend->white_tex);
    }
    if (rend->vg) {
        nvgDeleteGLES2(rend->vg);
    }
    if (rend->items) {
        item_t* item, * tmp;
        DL_FOREACH_SAFE(rend->items, item, tmp) {
    ...
    if (rend->grid_cache) {
        cache_clear(rend->grid_cache);
        free(rend->grid_cache);
        rend->grid_cache = NULL;
    }
}
