...
void json_remove_key(json_value* object, const char* key) {
    ...
    for (unsigned int i = 0; i < object->u.object.length; ++i) {
        if (strcmp(object->u.object.values[i].name, key) == 0) {
            // Free the memory of the value associated with the key
            json_value_free(object->u.object.values[i].value);

            // Free the memory allocated for the key name itself
            free((void*)object->u.object.values[i].name);

            // Shift elements to the left to remove the key-value pair
            for (unsigned int j = i; j < object->u.object.length - 1; ++j) {
                object->u.object.values[j] = object->u.object.values[j + 1];
            }

            // Decrement the length of the object
