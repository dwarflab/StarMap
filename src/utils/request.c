...
static char *create_local_path(const char *url, const char *suffix)
{
    ...
    char* t_url = strchr(url,'?');
    char* d_url = NULL;
    if(t_url){
        d_url = malloc(t_url - url + 1);
        memcpy(d_url,url,t_url - url);
        d_url[t_url - url] = '\0';
    }

    r = asprintf(&ret, "%s/%s%s", g.cache_dir, d_url ? : url, suffix ?: "");
    if(d_url) free(d_url);
    if (r == -1) LOG_E("Error");
    ...
    if (!req->local_path) {
        ...
        if (req->done && req->status_code / 100 == 2) {
            ...
            if (!file){
                ...
                printf("%s \n",req->url);
                printf("%s \n",req->local_path);
                printf("open file fail errno = %d, reason = %s \n", errNum, strerror(errNum));
                perror(NULL);
            }
            assert(file);
