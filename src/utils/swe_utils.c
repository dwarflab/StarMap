                           ...
                           int *w, int *h, int *bpp)
{
    ...
    if (WebPGetInfo(data, size, NULL, NULL)) {
        *bpp = 4;
        uint8_t* decode_data = WebPDecodeRGBA(data, size, w, h);
        size_t data_size = *w * *h * 4;
        uint8_t* res_data = malloc(data_size);
        memcpy(res_data, decode_data, data_size);
        WebPFree(decode_data);
        return res_data;
    }
    else {
        stbi_uc* decode_data = stbi_load_from_memory(data, size, w, h, bpp, *bpp);
        if (decode_data) {
            size_t data_size = *w * *h * *bpp;
            uint8_t* res_data = malloc(data_size);
            memcpy(res_data, decode_data, data_size);
            stbi_image_free(decode_data);
            return res_data;
        }
        else {
            return decode_data;
