...
int module_clear_data_source(obj_t* module) {
    int r;
    assert(module);
    if (!module->klass->clear_data_source) {
        return -1;
    }
    else {
        r = module->klass->clear_data_source(module);
        return r;
    }

}

int module_reload_data_source(obj_t* module)
{
    int r;
    assert(module);
    if (!module->klass->reload_data_source) {
        return -1;
    }
    else {
        r = module->klass->reload_data_source(module);
        return r;
    }
}
